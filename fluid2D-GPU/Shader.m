...
//    }
    ...
    NSString *theSource = [NSString stringWithContentsOfFile:file encoding:NSUTF8StringEncoding error:nil];
	...
	if (_defines != nil) {
		theSource = [_defines stringByAppendingFormat:@"%@", theSource];
	}

	source = (GLchar *)[[NSString stringWithString:theSource] UTF8String];
...
- (id) initWithShaderSettings:(NSDictionary*)settings defines:(NSArray *)defines;
{
	_defines = @"";
	for (NSString *str in defines) {
		_defines = [_defines stringByAppendingFormat:@"#define %@ 1\n", str];
	}
	
	NSLog(@"defines = %@", _defines);
	
	return [self initWithShaderSettings:settings];
}
...
{    
    ...
    if( self )
    {
        ...
        attributeHandles = [NSMutableDictionary dictionary];
        uniformHandles = [NSMutableDictionary dictionary];
